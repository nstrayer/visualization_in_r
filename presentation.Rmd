...
Even simpler you can simply do a stacked bar chart. 
...
So the hierarchy of the data is clearly visible but the intuitive interpretation of the bar is slightly confusing. A percentile is not a sum of values but simply a place on the continuum of a scale. Now we visualize it as a dot-plot.
...
p + geom_boxplot(fill = "steelblue", color = "steelblue") + labs(title = "Box Plots")
```
